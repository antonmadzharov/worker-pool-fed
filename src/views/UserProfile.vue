...
export default {
  ...
  methods: {
    ...
    async getUserAvatar() {
      const userData = await axios.get('/users/self/avatar', {
        headers: {
          Authorization: localStorage.getItem('JWT'),
        },
      })
      this.image = 'data:image/jpeg;base64,' + btoa(userData.data.results[0].image.data);
      console.log('the image', userData.data)
    },
  ...
  computed: {
    handleImage() {
      return this.isError ? assets.question : this.image
    },
  },
  async mounted() {
